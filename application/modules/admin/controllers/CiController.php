...
*/
    ...
    public function avaliarciAction()
    {
        ...
        $this->_helper->layout()->disableLayout();
        if($login_usuario == "ITDESK" && $debug){
            $ci = ($this->_request->getParam('id'));
        ...
        if ($this->_request->isPost() || ($login_usuario == "ITDESK" && $debug)) {
            ...
            if ($form->isValid($formData)){
                ...
                foreach ($aprovados as $key => $value) {
                    if($value["USUARIO"] == $id_usuario && $value["STATUS"] = $status){
                        $retorno = array(0 => "CI Já avaliada pelo usuário ".$login_usuario);
                        $this->view->mensagem = base64_encode("CI Já avaliada pelo usuário ".$login_usuario);
//                        return;
                    }
                }
                if(!isset($retorno)){
                    ...
                    if (strlen($retorno[0]) == 0 && $status == 3){
                        $arrayDadosCI['ID']     = $ci;
                        if(isset($aprovados[0]["STATUS"]) && $aprovados[0]["STATUS"] == 4){
                            $arrayDadosCI['STATUS'] = 4;
                            echo "Já está reprovado";
                        }elseif($necessario<=count($aprovados)){
                            $arrayDadosCI['STATUS'] = 3;
                        ...
                        if($login_usuario == "ITDESK" && $debug){
                            ...
                            Zend_Debug::dump($arrayDadosCI);
                            exit;
                            $retorno = array(0 => "teste apenas usuário ITDESK");
                        }else
                        $retorno = $objCI->save($arrayDadosCI);
                    }
                }
...
//                    $mensagem = 'enviada';
                    if($mensagem == 'enviada'){
                        ...
                        $date = New Zend_Date();
                        $conteudo = $date->get('dd/MM/yyyy HH:mm')." - ".$login.' | '.$mensagem;
                        $fp = fopen(substr(UPLOAD_PATH, 0, -7)."_logs".DIRECTORY_SEPARATOR.'ERRO_CI_ENVIO_EMAIL'.$date->get('-yyyy_MM').'.txt', "a");
...
//                                     'data'         => $CI['DATA'],
    ...
    public function cancelarciAction()
    {
        ...
        $objCilog       = new Admin_Model_Cilog();        
        $usuarioAprovou = $objCilog->usuarioAprovou($id);
        
        $CI             = $objCI->findCI($id);
        if($CI["USUARIO"] == $usuario_logado || $usuarioAprovou){
            $arrayDadosCI   = array();
            $arrayDados     = array();
            if ($this->_request->isPost()) {
                $formData       = $this->_request->getPost();
                ...
                if ($form_cancelar->isValid($formData)){                    
                    ...
                    if (strlen($retorno[0]) == 0){
                        ...
                        $retorno = $objCilog->save($arrayDados);
                    }
                    if (strlen($retorno[0]) == 0){
            ...
            if($value["LOGIN"]){
                if($value["EMAIL"]){
                    ...
                    if(COMPUTERNAME=="OCS030"){
                        ...
                        $para_email = $value["EMAIL"] ? $value["EMAIL"] : "anderson@usinasantafe.com.br";
                        $para_nome  = $value["NOME"] ? $value["NOME"] : $value["LOGIN"];
                    }
                }
